...
CPU_SRC := RCKangaroo.cpp GpuKang.cpp Ec.cpp utils.cpp TargetSet.cpp MultiTargetRelations.cpp
...
check-host: check-portable-ec check-quality-gates check-benchforge-rckmetal check-target-assignment check-cuda-loop-table-layout check-multi-target-relations
	python3 tests/check_multi_target_sharding_source.py
	sh tests/check_target_parser.sh

check-target-assignment:
	$(CC) -std=c++17 -O2 -I. tests/target_assignment_check.cpp TargetSet.cpp Ec.cpp utils.cpp -o /tmp/rckangaroo-target-assignment-check
	/tmp/rckangaroo-target-assignment-check
	rm -f /tmp/rckangaroo-target-assignment-check

check-cuda-loop-table-layout:
	$(CC) -std=c++17 -O2 -I. tests/cuda_loop_table_layout_check.cpp -o /tmp/rckangaroo-cuda-loop-table-layout-check
	/tmp/rckangaroo-cuda-loop-table-layout-check
	rm -f /tmp/rckangaroo-cuda-loop-table-layout-check

check-multi-target-relations:
	$(CC) -std=c++17 -O2 -I. tests/multi_target_relation_check.cpp MultiTargetRelations.cpp Ec.cpp utils.cpp -o /tmp/rckangaroo-multi-target-relation-check
	/tmp/rckangaroo-multi-target-relation-check
	rm -f /tmp/rckangaroo-multi-target-relation-check
...
macos-ecint-divstep-source-check:
	python3 tests/check_ecint_divstep_source.py
...
macos-metal-precompiled-library-source-check:
	python3 tests/check_metal_precompiled_library_source.py
...
macos-metal-target-lookup-source-check:
	python3 tests/check_metal_target_lookup_source.py

macos-metal-target-lookup-compact-source-check:
	python3 tests/check_metal_target_lookup_compact_source.py

macos-metal-target-lookup-tag32-source-check:
	python3 tests/check_metal_target_lookup_tag32_source.py
...
macos-check: check-host check-autoresearch check-quality-gates macos-lto-flags-check macos-jump-index-source-check macos-ecint-carry-source-check macos-ecint-divstep-source-check macos-hotpath-microbatch-source-check macos-affine-z-check-source-check macos-affine-inplace-field-source-check macos-affine-reverse-loop-source-check macos-metal-precompiled-library-source-check macos-metal-dp4-uchar-infinity-source-check macos-metal-dp4-q-struct-row-source-check macos-metal-dynamic-jump-walk-source-check macos-metal-dynamic-compact-dp-source-check macos-metal-dynamic-dp-stream-source-check macos-metal-dynamic-dp-stream-mask-source-check macos-metal-dynamic-dp-stream-inplace-source-check macos-metal-dynamic-dp-stream-xyzz-source-check macos-metal-dynamic-dp-count-source-check macos-metal-target-lookup-source-check macos-metal-target-lookup-compact-source-check macos-metal-target-lookup-tag32-source-check macos-metal-affine-scan-target-lookup-source-check macos-build
	...
	sh tests/check_target_lookup_tag32_parity_parallel_insert_cli.sh
...
macos-metal-jacobian-dynamic-dp-stream-xyzz-chain-steps512-bench: macos-build
	./$(MACOS_TARGET) metal-jacobian-dynamic-dp-stream-xyzz-chain-bench --iterations 16384 --steps 512 --packets 2 --jumps 16 --dp-bits 8 --min-ms 200

macos-metal-jacobian-dynamic-dp-stream-xyzz-persistent-chain-steps512-bench: macos-build
	./$(MACOS_TARGET) metal-jacobian-dynamic-dp-stream-xyzz-persistent-chain-bench --iterations 16384 --steps 512 --packets 2 --rounds 2 --jumps 16 --dp-bits 8

macos-metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-bench: macos-build
	./$(MACOS_TARGET) metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-bench --iterations 16384 --steps 512 --jumps 16 --dp-bits 8 --min-ms 200

macos-metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-target-lookup-tag32-bench: macos-build
	./$(MACOS_TARGET) metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-target-lookup-tag32-bench --iterations 16384 --steps 512 --jumps 16 --dp-bits 8 --target-count 1048576 --hits 64 --min-ms 200
...
macos-metal-target-lookup-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-bench --target-count 1048576 --query-count 1048576 --hits 4096 --min-ms 500

macos-metal-target-lookup-compact-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-compact-bench --target-count 1048576 --query-count 1048576 --hits 4096 --min-ms 500

macos-metal-target-lookup-tag32-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-tag32-bench --target-count 1048576 --query-count 1048576 --hits 4096 --min-ms 500

macos-target-lookup-tag32-parity-parallel-insert-bench: macos-build
	./$(MACOS_TARGET) target-lookup-tag32-parity-parallel-insert-bench --target-count 1048576 --injected-count 64 --iterations 1

macos-metal-target-lookup-tag32-filter-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-tag32-filter-bench --target-count 25005000 --query-count 1082368 --hits 64 --min-ms 500

macos-metal-target-lookup-tag32-filter-persistent-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-tag32-filter-persistent-bench --target-count 25005000 --query-count 1082368 --hits 64 --min-ms 700

macos-metal-target-lookup-tag16-filter-persistent-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-tag16-filter-persistent-bench --target-count 25005000 --query-count 1082368 --hits 64 --min-ms 700

macos-metal-target-lookup-tag16-hash-filter-persistent-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-tag16-hash-filter-persistent-bench --target-count 25005000 --query-count 1082368 --hits 64 --min-ms 700

macos-metal-target-lookup-tag32-persistent-bench: macos-build
	./$(MACOS_TARGET) metal-target-lookup-tag32-persistent-bench --target-count 1048576 --query-count 1048576 --hits 4096 --min-ms 500
