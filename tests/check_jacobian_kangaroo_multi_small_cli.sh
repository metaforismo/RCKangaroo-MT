...
case "$output" in
	*"FOUND private_key=7 private_key_hex=7"*"target_index=1"*"method=jacobian_kangaroo_multi_small"*"architecture=shared_tame"*"dp_lookup=hash"*"affine_conversion=batch"*"target_count=2"*"tame_states=1"*"wild_states=2"*)
		exit 0
		;;
	*)
		printf '%s\n' "$output"
