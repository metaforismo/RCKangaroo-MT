...
steps256_output="$(./macos/rck_macos metal-jacobian-dynamic-dp-stream-inplace-bench --iterations 128 --steps 256 --jumps 8 --dp-bits 8 --min-ms 1 2>&1)"
steps256_status=$?
if [ "$steps256_status" -ne 0 ]; then
	printf 'metal-jacobian-dynamic-dp-stream-inplace-bench steps256 returned status %s\n' "$steps256_status"
	printf '%s\n' "$steps256_output"
	exit 1
fi

case "$steps256_output" in
	*"\"backend\":\"metal\""*"\"operation\":\"jacobian_affine_walk_dynamic_dp_stream_inplace\""*"\"sample_count\":128"*"\"steps_per_sample\":256"*"\"jump_count\":8"*"\"jump_index\":\"power2_mask\""*"\"jump_mixer\":\"avalanche64\""*"\"output_layout\":\"dp_stream\""*"\"output_bytes_per_record\":20"*"\"emitted_records\":"*"\"dp_stream_overflow\":false"*"\"distance_tracking\":\"dp_stream_uint64\""*"\"dp_tracking\":\"projective_x_limb0\""*"\"dp_bits\":8"*"\"dp_count\":"*"\"dp_checksum\":\"0x"*"\"correctness\":true"*)
		;;
	*"\"backend\":\"metal\""*"\"operation\":\"jacobian_affine_walk_dynamic_dp_stream_inplace\""*"\"sample_count\":128"*"\"steps_per_sample\":256"*"\"jump_count\":8"*"\"jump_index\":\"power2_mask\""*"\"output_layout\":\"dp_stream\""*"\"skipped\":true"*"\"reason\":\"no Metal device available\""*)
		;;
	*)
		printf '%s\n' "unexpected metal-jacobian-dynamic-dp-stream-inplace-bench steps256 output"
		printf '%s\n' "$steps256_output"
		exit 1
		;;
esac

overflow_output="$(./macos/rck_macos metal-jacobian-dynamic-dp-stream-inplace-bench --iterations 128 --steps 256 --jumps 32 --dp-bits 8 --min-ms 1 2>&1)"
overflow_status=$?
if [ "$overflow_status" -ne 0 ]; then
	printf 'metal-jacobian-dynamic-dp-stream-inplace-bench overflow guard returned status %s\n' "$overflow_status"
	printf '%s\n' "$overflow_output"
	exit 1
fi

case "$overflow_output" in
	*"\"backend\":\"metal\""*"\"operation\":\"jacobian_affine_walk_dynamic_dp_stream_inplace\""*"\"steps_per_sample\":256"*"\"jump_count\":32"*"\"correctness\":false"*"\"reason\":\"in-place dynamic dp stream packet distance exceeds uint32 accumulator\""*)
		;;
	*)
		printf '%s\n' "unexpected metal-jacobian-dynamic-dp-stream-inplace-bench overflow guard output"
		printf '%s\n' "$overflow_output"
		exit 1
		;;
esac
