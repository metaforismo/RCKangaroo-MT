...
checks = [
    ...
    ("wild loop reset kernel exists", "KernelResetWildLoopState(const TKparams Kparams)" in gpu_core),
    ("host calls wild loop reset on preserve tame", "CallGpuKernelResetWildLoopState(Kparams)" in gpu_cpp),
    ("wild loop reset launch exists", "KernelResetWildLoopState << < Kparams.BlockCnt, Kparams.BlockSize, 0 >> > (Kparams)" in gpu_core),
    ("cycle reset does not globally clear loop table", "if (!preserve_tame)\n\t{\n\t\terr = cudaMemset(Kparams.LoopTable" in gpu_cpp),
