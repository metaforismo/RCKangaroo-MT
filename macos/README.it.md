...
`macos-jacobian-kangaroo-multi-small-bench` genera target sintetici deterministici, mette un target risolvibile all'ultimo indice e misura solve tiny multi-target shared-tame al secondo con lookup DP a bucket hash. Esegue anche un baseline single-target con gli stessi parametri e riporta `single_target_ops_per_sec`, `speedup_vs_single` e `target_throughput_vs_single`; l'ultimo campo moltiplica i solve multi al secondo per il numero di target prima del confronto con il baseline single-target. Usa `--target-count` per confrontare 1, 2, 4, 8 o piu' target mantenendo range bounded e parametri jump uguali.
