...
Il comando fixed-round accetta anche `--walk-round-mode independent|persistent`. Il default `independent` mantiene il batching promosso in un solo dispatch con start round deterministici distinti. `persistent` fa avanzare gli stessi stati walker attraverso i round, registra `walk_round_mode=persistent` e `distance_tracking=round_cumulative_uint64`, e valida ogni confine round con un oracle CPU cumulativo. Trattalo come probe architetturale piu' simile a un solver, non come default di velocita' attuale: i gate locali su M3 hanno tenuto `independent` davanti sul tempo di walk anche quando i numeri wall inclusivi del setup erano rumorosi.
