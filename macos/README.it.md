...
Per `--lookup-query-mode distinct-misses` fixed-round, il path tag16/Bloom64 hash-filter puo' riportare `query_input=hash64_prefix_gpu_miss`. In questa modalita' l'host valida esattamente il suffisso deterministico di miss primari, invia a Metal solo gli hash del prefisso DP reale, e lascia alla GPU derivare gli hash dei miss deterministici dentro il kernel filtro. Se la validazione trovasse un miss primario che e' un target esatto, il codice tornerebbe al path raw con buffer fisico di hash e retry.
