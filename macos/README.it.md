...
Gli stessi benchmark tiny CPU accettano anche `--jump-schedule scaled4-probe-power2` con `--jumps 4`. E' un probe portfolio lato solver: prova prima una breve corsa `scaled4-balanced`, oggi `min(max_steps, max(10000, max_steps / 200))` passi, poi cade su una tabella `power2` a 16 jump se il probe non risolve. Il JSON riporta `portfolio_probe_max_steps`, `portfolio_probe_hits`, `portfolio_fallback_runs`, `last_portfolio_probe_dp_count` e i campi della tabella fallback, cosi' il costo del probe non viene nascosto. Questo percorso serve per ricerca schedule su oracle CPU tiny; non e' il gate Metal default.
