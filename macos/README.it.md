...
Gli stessi benchmark tiny CPU accettano anche `--jump-schedule scaled4-probe-power2` con `--jumps 4`. E' un probe portfolio lato solver: prova prima una breve corsa `scaled4-balanced`, di default `min(max_steps, max(10000, max_steps / 200))` passi, poi cade su una tabella `power2` a 16 jump se il probe non risolve. Usa `--portfolio-probe-steps N` per misurare esplicitamente quel budget iniziale. Il JSON riporta `portfolio_probe_max_steps`, `portfolio_probe_hits`, `portfolio_fallback_runs`, `last_portfolio_probe_dp_count` e i campi della tabella fallback, cosi' il costo del probe non viene nascosto. Questo percorso serve per ricerca schedule su oracle CPU tiny; non e' il gate Metal default.
...
`make macos-build` prova anche a costruire una libreria sidecar ignorata da git in `macos/rck_macos.metallib` piu' `macos/rck_macos.metallib.meta`, partendo dallo stesso sorgente `MetalFieldKernels.h`, quando `xcrun metal` e `xcrun metallib` sono disponibili. La sidecar viene compilata con `MACOS_METAL_FLAGS ?= -finline-functions`, sovrascrivibile per esperimenti di toolchain. A runtime la sidecar viene caricata automaticamente solo quando l'hash nel metadata combacia con il sorgente Metal embedded; metadata mancanti o stale fanno fallback alla compilazione runtime da sorgente. Per provarla in modo esplicito anche senza metadata coerente usa `RCK_METAL_USE_PRECOMPILED=1`; per caricare una libreria specifica usa `RCK_METAL_FIELD_LIB=/path/to/rck_macos.metallib`; per forzare il percorso sorgente usa `RCK_METAL_DISABLE_PRECOMPILED=1`. Se il Metal Toolchain non e' disponibile, il build rimuove sidecar stale e il binario continua a usare il fallback runtime da sorgente.
...
`metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-bench` e' il primo probe DP affine al confine packet. Il kernel Metal esegue il walk dinamico XYZZ e scrive una distanza packet a 64 bit per walker; poi l'host normalizza in batch `X,Y,ZZ,ZZZ` con una sola inversione sui prodotti `ZZ*ZZZ`, scansiona i bit bassi di `x` affine e riporta `dp_tracking=affine_x_limb0_cpu_batch`. Cosi' il predicato DP resta piu' vicino a un solver reale senza introdurre inversioni per step. Il JSON separa throughput GPU grezzo (`gpu_ops_per_sec`) e throughput end-to-end packet-piu'-affine-scan (`ops_per_sec`) e registra `affine_scan_seconds`, cosi' i prossimi lavori possono spostare la normalizzazione su Metal senza nasconderne il costo. Il path affine-scan supporta anche `--steps 1024` con `--dp-bits 7`, `--steps 2048` con `--dp-bits 6` e il probe solo riproducibilita' `--steps 4096` con `--dp-bits 5`. I risultati paired locali tengono 2048/dp6 come plateau e scartano 4096/dp5. Per quel plateau 2048/dp6, un gate locale setup-inclusive ha accettato `--jumps 4 --jump-schedule scaled4-balanced` contro lo schedule `power2` a 16 jump; trattarlo come probe riproducibile di schedule finche' altre macchine non lo confermano.

Il JSON affine-scan riporta anche `dp_sampling=packet_endpoint` e `dp_normalization=host_batch_affine`. Sono campi anti-cheat intenzionali: i gate veloci correnti normalizzano gli endpoint dei packet, non ogni step interno, quindi una futura pipeline DP per-step o full-state dovra' dichiarare uno scope diverso.

`metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-target-lookup-tag32-bench` collega quella superficie DP affine al gate multi-target compatto esatto. La scan host emette `x256` affine completo piu' parita' di `y` per candidati DP reali al confine packet, inietta un numero controllato di quelle chiavi in una tabella target tag32, e di default lancia il kernel Metal `target_lookup_tag32_exact256` sul set di query DP. Il JSON runtime riporta `output_layout=affine_dp_scan_target_lookup`, `dp_tracking=affine_x256_y_parity_cpu_batch`, `target_key=x256_y_parity`, `candidate_verification`, `dp_query_count`, `lookup_repeat`, `lookup_query_mode`, `lookup_engine`, `lookup_engine_effective`, `query_count`, `injected_hits`, `hit_count`, `miss_count`, `lookup_seconds`, `lookup_hash_seconds`, `lookup_gpu_seconds`, `lookup_exact_seconds`, `gpu_lookup_lookups_per_sec`, `lookups_per_sec` e `target_lookup_checksum`. `--lookup-engine cpu` lascia invariati walk GPU e scan affine, ma sposta sull'host CPU solo la join target tag32 esatta finale. `--lookup-engine gpu-filter` usa su Metal un filtro tag32 da 4 byte per bucket e poi verifica su CPU solo i positivi compatti con equality esatta `x256 + y_parity`; il JSON riporta `lookup_layout=open_address_tag32_filter_exact256`, `filter_positive_count` e `filter_false_positive_count`. `--lookup-engine gpu-filter16-hash` usa il filtro tag16 da 2 byte e hash query precomputati da 64 bit, poi risolve i positivi compatti con la stessa equality CPU esatta; il JSON riporta `lookup_layout=open_address_tag16_hash_filter_exact256`, `query_input=hash64`, `target_query_hash_bytes` e gli stessi contatori positivi/falsi positivi. `--lookup-engine gpu-filter16-hash-repeat` vale solo in modalita' repeat: invia a Metal un solo batch base di hash DP, lancia una griglia tag16 2D `(base_query, repeat)` e riporta `query_input=hash64_repeat_indexed`; quando base-query e repeat stanno entrambi in 16 bit, la GPU emette positivi packed `(repeat_id, base_query_id)`, cosi' il resolver CPU esatto evita divisione/modulo sull'indice logico. Le shape integrate sparse-repeat piu' grandi possono emettere positivi `base_query_count_repeated`: la GPU continua a sondare ogni repeat logico, mentre il resolver sparse riceve contatori per base query e continua a contare ogni hit o falso positivo logico ripetuto. I path full-output conservano gli indici logici. Il JSON registra il path scelto come `repeat_positive_index_encoding`. Per batch accumulati di almeno 2,097,152 query, il builder host degli hash query passa da seriale a parallelo sogliato, mantenendo visibile `lookup_hash_seconds`. `--lookup-engine auto` lascia visibile la richiesta come `lookup_engine=auto`, poi registra il percorso scelto in `lookup_engine_effective`; la policy attuale lascia su CPU i grandi batch limitati da 25M target finche' un gate paired end-to-end non conferma il filtro, e usa ancora GPU con cap lookup a 512 thread per tabelle piu' cache-friendly quando il batch accumulato raggiunge almeno un milione di query. `--lookup-tg-limit N` regola solo il kernel Metal finale di target lookup, lasciando invariata la policy threadgroup del walk XYZZ. `--lookup-repeat N` espande il batch reale di query DP affini prima del lookup, cosi' il benchmark puo' modellare un solver che accumula DP al confine packet prima di lanciare una join target piu' grande. Il default `--lookup-query-mode repeat` ripete il batch reale di DP e ripete gli indici target esatti attesi dello stesso fattore. `--lookup-query-mode distinct-misses` conserva un batch reale di DP, poi riempie gli slot bulk rimanenti con chiavi deterministiche verificate come miss nella tabella target tag32; cosi' si ottiene un probe di join target mostly-miss piu' realistico per la cache, mantenendo un oracle esatto di hit/miss. Il default `N=1` e `--lookup-engine gpu` conservano il gate end-to-end originale. I gate autoresearch `bulk1024`, `distinct_misses1024`, `lookup_tg512`, `gpu_filter25m`, `tag16_hash_filter25m_tg256_gpu_lookup`, `tag16_hash_filter25m_parallel_hash_repeat4096`, `tag16_hash_filter25m_repeat_indexed2048`, `tag16_hash_filter25m_repeat_packed_positive2048`, `steps2048_dp6_setup` e `scaled4_j4_setup` separano volutamente throughput lookup, throughput walk, distribuzione dello schedule jump e costo di verifica esatta. Il comando companion `metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-target-lookup-tag32-rounds-bench` costruisce batch di walk deterministici distinti per `--rounds`, mette quegli start in un solo dispatch Metal piu' grande, divide l'output di nuovo in scan affini e offset target per round, inietta hit per ogni round in una sola tabella/filtro target condivisa, concatena tutte le chiavi DP dei round in un solo dispatch lookup repeat aggregato, e riporta campi aggregati `round_count`, DP, lookup, hit, falsi positivi e checksum; usalo come probe anti-cheat steady-state piu' severo prima di promuovere claim di velocita' multi-target. Le opzioni `--lookup-filter-bits 16|32` e `--lookup-filter-mode tag16|tag16-mix|tag32` mantengono il filtro hash-repeat tag16 da 2 byte di default, provano un filtro hash-repeat tag16 mixed della stessa dimensione, oppure passano a un filtro hash-repeat tag32 da 4 byte; ogni modalita' continua comunque a risolvere i positivi con equality CPU esatta `x256 + y_parity`. `--lookup-repeat-mode dedup` e' una diagnostica esplicita non-default per batch ripetuti: filtra ogni query DP base una sola volta, riespande gli output esatti nello stesso oracle logico repeat, e riporta `physical_query_count` piu' `physical_filter_positive_count`; confrontarla solo come limite superiore per query ripetute, non come throughput su query distinte. E' un benchmark integrato non-cheating per la join multi-target sul path macOS: throughput del walk, costo della scan affine, filtro target ed equality finale restano visibili.

Il JSON target-lookup affine-scan riporta anche `target_lookup_scope=packet_endpoint_dp_keys`, chiarendo che la join multi-target compatta consuma le chiavi DP agli endpoint packet emesse dalla scan affine host.

Il comando fixed-round accetta anche `--walk-round-mode independent|persistent`. Il default `independent` mantiene il batching promosso in un solo dispatch con start round deterministici distinti. `persistent` fa avanzare gli stessi stati walker attraverso i round, scrive ogni confine round e distanza cumulativa direttamente dal kernel Metal, registra `walk_round_mode=persistent` e `distance_tracking=round_cumulative_uint64`, e valida ogni confine con un oracle CPU cumulativo. Trattalo come probe architetturale piu' simile a un solver, non come default automatico: i gate locali su M3 sono vicini e rumorosi, quindi la promozione a default richiede ancora il gate paired 25M canonico.

Il JSON target-lookup riporta anche `mean_jump_distance`, `gpu_distance_per_sec`, `distance_per_sec`, `setup_inclusive_distance_per_sec` e `setup_inclusive_wall_distance_per_sec`. Questi campi moltiplicano i rate di operazioni di gruppo per la distanza media dello schedule, cosi' i probe sugli schedule possono confrontare la distanza kangaroo effettiva coperta al secondo invece di ottimizzare solo il rate grezzo di operazioni.

Per `--lookup-query-mode distinct-misses` fixed-round, il path tag16/Bloom64 hash-filter puo' riportare `query_input=hash64_prefix_gpu_miss`. In questa modalita' l'host invia a Metal solo gli hash del prefisso DP reale e lascia alla GPU derivare gli hash dei miss deterministici dentro il kernel filtro. Il resolver CPU esatta i positivi del prefisso DP reale e i soli miss generati che passano il filtro GPU; se un suffisso generato risultasse un target esatto, il codice tornerebbe al path raw con buffer fisico di hash e retry.

Il comando fixed-round accetta anche `--lookup-query-mode distinct-misses`. In questa modalita' conserva le chiavi DP reali una sola volta, riempie il resto del batch fisico con chiavi deterministiche verificate come miss nella tabella target compatta x-only piu' parita', lancia il kernel tag16 hash-filter non-repeat su ogni query fisica, risolve i positivi con equality esatta `x256 + y_parity`, e valida tutto l'output contro indici attesi espliciti. La diagnostica `--lookup-filter-mode tag16-mix` e' supportata anche nello stesso path distinct, ma non e' una policy di default dopo che il gate locale M3 25M ha misurato piu' falsi positivi e nessun guadagno di velocita'. Usa distinct-misses per controllare se un guadagno in repeat-mode sopravvive a uno stream DP mostly-miss piu' realistico; resta volutamente separato da `--lookup-repeat-mode dedup`, che e' solo una diagnostica upper-bound per query ripetute.
...
Per i grandi output sparse-repeat base-count, il kernel repeat tag16 standard lancia come `(repeat, base_query)` e riduce i positivi dentro ogni threadgroup prima di una sola add globale per blocco. Continua a sondare ogni repeat logico e conserva `base_query_count_repeated`, ma evita la vecchia pressione atomica globale per ogni positivo sulle basi ripetute.
...
Su run dominati dal setup delle tabelle target, `RCK_TARGET_SETUP_WORKERS=N`
e' una manopola riproducibile esplicita solo per la costruzione delle tabelle.
Non cambia il worker count di validazione/replay, a meno che venga impostato
anche `RCK_VALIDATION_WORKERS`. Il JSON riporta `target_setup_workers` accanto a
`validation_workers`, cosi' i gate paired possono distinguere il tuning della
tabella target dal resto del percorso. I controlli locali su M3 non hanno
giustificato un nuovo default automatico; lascia il worker count di default a
meno che un gate paired sul comando esatto dimostri il contrario.
...
Il benchmark affine-scan integrato espone anche `--lookup-engine gpu-filter16-hash`: usa lo stesso filtro tag16 e gli stessi hash query precomputati nel join finale dei DP, ma mantiene invariati walk, scansione affine, iniezione hit controllata e verifica exact CPU sui positivi compatti. Il JSON riporta `lookup_layout=open_address_tag16_hash_filter_exact256`, `query_input=hash64` e `target_query_hash_bytes`. Sopra 2,097,152 query accumulate usa un builder host parallelo per gli hash query; sotto quella soglia conserva il builder seriale. La variante esplicita `--lookup-engine gpu-filter16-hash-repeat` e' solo per `--lookup-query-mode repeat`: legge gli hash base una volta, usa una griglia Metal 2D per gli indici ripetuti, emette positivi packed quando le dimensioni stanno in 16 bit, e conserva lo stesso checksum/oracle esatto.
