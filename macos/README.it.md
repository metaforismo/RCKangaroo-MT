...
`macos-jacobian-batch-affine-bench` isola il percorso batch inversion usato dal solver multi-target shared-tame. Costruisce un punto tame Jacobian piu' punti wild Jacobian configurabili, converte l'intero batch ad affine con una sola inversione di campo per iterazione, valida ogni punto affine contro riferimenti scalari, riporta `field_rhs_passing=const_ref`, `affine_z_access=const_ref`, `affine_buffer=resize_reuse`, `affine_active_path=all_active_fast` e `affine_tail_update=skip_final`, e riporta conversioni batch al secondo piu' punti affini al secondo.
