...
Il percorso DP8 stream in-place ha anche specializzazioni packet `steps=16` e `steps=32`. Eseguono piu' salti dinamici per thread, salvano lo stato Jacobian aggiornato nel buffer di input, e validano sia lo stream DP sparso sia lo stato finale contro replay CPU. Queste modalita' sono utili per tuning di packet in walk GPU persistenti perche' ammortizzano load/store dello stato su piu' operazioni di gruppo; controllano il predicato DP solo al confine del packet.
