...
kernel void jacobian_affine_walk_dynamic_xyzz_steps2048_pow2_u32_distance(device ulong* p_xyzz [[buffer(0)]],
                                                                          constant AffineJumpValue* q_xy [[buffer(1)]],
                                                                          device uchar* p_infinity [[buffer(2)]],
                                                                          device ulong* out_distances [[buffer(4)]],
                                                                          constant uint& count [[buffer(6)]],
  ...
  XyzzDistanceValue out = xyzz_walk_pow2_u32_distance(p_xyzz[p_base + 0], p_xyzz[p_base + 1], p_xyzz[p_base + 2], p_xyzz[p_base + 3],
                                                     ...
                                                     p_xyzz[p_base + 8], p_xyzz[p_base + 9], p_xyzz[p_base + 10], p_xyzz[p_base + 11],
                                                     p_xyzz[p_base + 12], p_xyzz[p_base + 13], p_xyzz[p_base + 14], p_xyzz[p_base + 15],
                                                     p_infinity[id], q_xy, jump_distances, jump_mask, 2048);
  store_xyzz_distance_value(p_xyzz, p_base, out);
  p_infinity[id] = out.inf ? 1 : 0;
  out_distances[id] = (ulong)out.distance;
}

kernel void jacobian_affine_walk_dynamic_xyzz_steps4096_pow2_u32_distance(device ulong* p_xyzz [[buffer(0)]],
                                                                          constant AffineJumpValue* q_xy [[buffer(1)]],
                                                                          device uchar* p_infinity [[buffer(2)]],
                                                                          device ulong* out_distances [[buffer(4)]],
                                                                          constant uint& count [[buffer(6)]],
  ...
  XyzzDistanceValue out = xyzz_walk_pow2_u32_distance(p_xyzz[p_base + 0], p_xyzz[p_base + 1], p_xyzz[p_base + 2], p_xyzz[p_base + 3],
                                                     ...
                                                     p_xyzz[p_base + 8], p_xyzz[p_base + 9], p_xyzz[p_base + 10], p_xyzz[p_base + 11],
                                                     p_xyzz[p_base + 12], p_xyzz[p_base + 13], p_xyzz[p_base + 14], p_xyzz[p_base + 15],
                                                     p_infinity[id], q_xy, jump_distances, jump_mask, 4096);
  store_xyzz_distance_value(p_xyzz, p_base, out);
  p_infinity[id] = out.inf ? 1 : 0;
  out_distances[id] = (ulong)out.distance;
}
...
kernel void target_lookup_tag16_hash_filter256(device const ushort* target_filter_buckets [[buffer(0)]],
...
kernel void target_lookup_tag16_hash_filter_repeat_base2d256(device const ushort* target_filter_buckets [[buffer(0)]],
                                                             device const ulong* base_query_hashes [[buffer(1)]],
                                                             device uint* out_positive_query_indices [[buffer(2)]],
                                                             device atomic_uint* out_filter_positive_count [[buffer(3)]],
                                                             constant uint& bucket_count [[buffer(4)]],
  ...
  while (probes < bucket_count) {
    ...
    if (bucket_tag == filter_tag) {
      uint out_slot = atomic_fetch_add_explicit(out_filter_positive_count, 1U, memory_order_relaxed);
      if (out_slot < query_count) {
        out_positive_query_indices[out_slot] = base_query_id;
      }
      break;
    }
    slot = (slot + 1U) & (bucket_count - 1U);
...
kernel void target_lookup_tag16_hash_filter_repeat_base_count2d256(device const ushort* target_filter_buckets [[buffer(0)]],
                                                                   device const ulong* base_query_hashes [[buffer(1)]],
...
kernel void target_lookup_tag16_mixed_hash_filter_repeat2d256(device const ushort* target_filter_buckets [[buffer(0)]],
                                                              device const ulong* base_query_hashes [[buffer(1)]],
                                                              device uint* out_positive_query_indices [[buffer(2)]],
                                                              device atomic_uint* out_filter_positive_count [[buffer(3)]],
                                                              constant uint& bucket_count [[buffer(4)]],
...
kernel void target_lookup_tag16_mixed_hash_filter_repeat_packed2d256(device const ushort* target_filter_buckets [[buffer(0)]],
                                                                     device const ulong* base_query_hashes [[buffer(1)]],
                                                                     device uint* out_positive_query_indices [[buffer(2)]],
                                                                     device atomic_uint* out_filter_positive_count [[buffer(3)]],
                                                                     constant uint& bucket_count [[buffer(4)]],
