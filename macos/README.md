...
For fixed-round `--lookup-query-mode distinct-misses`, the tag16/Bloom64 hash-filter path can report `query_input=hash64_prefix_gpu_miss`. In that mode the host exact-validates the deterministic primary miss suffix, sends only the real DP-prefix hashes to Metal, and lets the GPU derive deterministic suffix miss hashes inside the filter kernel. If validation ever finds a primary miss that is an exact target, the code falls back to the raw physical hash-buffer retry path.
