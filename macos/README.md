...
`metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-target-lookup-tag32-bench` connects that affine DP surface to the exact compact multi-target lookup gate. The host scan emits full affine `x256` plus `y` parity for real packet-boundary DP candidates, injects a controlled number of those keys into a tag32 target table, and runs the Metal `target_lookup_tag32_exact256` kernel against the DP query set. Runtime JSON reports `output_layout=affine_dp_scan_target_lookup`, `dp_tracking=affine_x256_y_parity_cpu_batch`, `target_key=x256_y_parity`, `candidate_verification=tag32_prefilter_then_exact_key_equality`, `dp_query_count`, `lookup_repeat`, `query_count`, `injected_hits`, `hit_count`, `miss_count`, `lookup_seconds`, `lookups_per_sec`, and `target_lookup_checksum`. `--lookup-repeat N` repeats the real affine-DP query batch before the lookup kernel so the benchmark can model a solver that accumulates packet-boundary DPs before launching a larger target join; the oracle repeats the expected exact target indices by the same factor, so hit and miss counts remain fully checked. The default `N=1` keeps the original end-to-end gate. The `bulk1024` autoresearch gate intentionally optimizes `lookups_per_sec`, not walk `ops_per_sec`, because it answers the separate batching question: how much target-lookup throughput is lost when we launch tiny DP query sets. This is an integrated non-cheating multi-target join benchmark for the macOS path: walk throughput, affine scan cost, and target lookup cost remain separated.
