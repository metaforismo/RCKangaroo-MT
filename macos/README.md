...
`macos-jacobian-batch-affine-bench` isolates the batch inversion path used by the shared-tame multi-target solver. It builds one tame Jacobian point plus configurable wild Jacobian points, converts the full batch to affine with one field inversion per iteration, validates every affine point against scalar references, reports `field_rhs_passing=const_ref`, `affine_z_access=const_ref`, `affine_buffer=resize_reuse`, `affine_active_path=all_active_fast`, and `affine_tail_update=skip_final`, and reports batch conversions per second plus affine points per second.
