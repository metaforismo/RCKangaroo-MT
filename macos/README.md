...
`jacobian_affine_walk_dynamic_dp_stream_xyzz` is a separate 256-step DP8 packet architecture that stores state as `X,Y,ZZ,ZZZ` instead of `X,Y,Z`. It updates `ZZ` and `ZZZ` directly in the mixed-add formula, avoiding a per-step recomputation of `Z^2` and `Z^3`, and validates both the sparse DP stream and final XYZZ state against a CPU XYZZ replay oracle. Because the state no longer stores `Z`, the jump mixer uses the same avalanche structure with `ZZ0` in place of `Z0`; the operation is reported separately from the Jacobian in-place packet. Run it with `metal-jacobian-dynamic-dp-stream-xyzz-bench --steps 256 --jumps 16 --dp-bits 8`.
