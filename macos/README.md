...
The fixed-round command also accepts `--lookup-query-mode distinct-misses`. In that mode it keeps the real DP keys once, fills the remaining physical query batch with deterministic keys verified to miss the exact compact x-only-plus-parity target table, runs the non-repeat tag16 hash-filter kernel against every physical query, resolves positives with exact `x256 + y_parity` equality, and validates the full output against explicit expected indices. Use it when checking whether a repeat-mode gain survives a more realistic mostly-miss DP stream; it is intentionally separate from `--lookup-repeat-mode dedup`, which remains only a repeated-query upper-bound diagnostic.
