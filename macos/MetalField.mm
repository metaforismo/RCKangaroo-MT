	...
	MetalDispatchStats* dispatch_stats)
{
	...
	if (p.empty() || jumps.empty() || jumps.size() != jump_distances.size() || (steps_per_sample != 8 && steps_per_sample != 16 && steps_per_sample != 32 && steps_per_sample != 64 && steps_per_sample != 128) || dp_bits != 8 || !IsMetalPowerOfTwo((unsigned int)jumps.size()) || jumps.size() > 32)
	{
	...
	@autoreleasepool
	{
		...
		const char* function_name = steps_per_sample == 128
...
bool RCKMetalJacobianDynamicDpStreamInplaceSelfTest(std::string& error)
{
	...
	for (unsigned int steps_per_sample : {8U, 16U, 32U, 64U, 128U})
	{
		std::vector<CpuJacobianPoint> state_out;
		std::vector<uint32_t> out_indices;
		std::vector<uint64_t> out_distances;
...
std::string RCKMetalJacobianDynamicDpStreamInplaceBenchJson(unsigned int iterations, unsigned int steps_per_sample, unsigned int jump_count, unsigned int min_ms, unsigned int threadgroup_limit, unsigned int dp_bits)
{
	...
	if ((steps_per_sample != 8 && steps_per_sample != 16 && steps_per_sample != 32 && steps_per_sample != 64 && steps_per_sample != 128) || dp_bits != 8 || !IsMetalPowerOfTwo(jump_count))
	{
		std::string reason = "in-place stream dynamic dp supports steps=8, steps=16, steps=32, steps=64, or steps=128, power-of-two jumps, dp_bits=8";
