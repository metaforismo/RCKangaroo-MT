...
static void PrintUsage()
{
	...
	printf("  rck_macos metal-target-lookup-tag32-filter-bench --target-count N --query-count N [--hits N] [--min-ms N] [--tg-limit N]\n");
	printf("  rck_macos metal-target-lookup-tag32-filter-persistent-bench --target-count N --query-count N [--hits N] [--min-ms N] [--tg-limit N]\n");
	printf("  rck_macos metal-target-lookup-tag16-filter-persistent-bench --target-count N --query-count N [--hits N] [--min-ms N] [--tg-limit N]\n");
	printf("  rck_macos metal-target-lookup-tag16-hash-filter-persistent-bench --target-count N --query-count N [--hits N] [--min-ms N] [--tg-limit N]\n");
	...
	printf("  rck_macos target-lookup-tag32-parity-parallel-insert-bench --target-count N [--injected-count N] [--iterations N]\n");
...
int main(int argc, char* argv[])
{
	...
	else if (strcmp(argv[1], "metal-target-lookup-tag32-filter-bench") == 0)
	{
		const char* target_count_s = NULL;
		const char* query_count_s = NULL;
		const char* hits_s = NULL;
		...
		if (ReadOption(argc, argv, "--hits", &hits_s))
		{
			...
			hits = query_count / 64U;
		printf("%s\n", RCKMetalTargetLookupTag32FilterBenchJson(target_count, query_count, hits, min_ms, threadgroup_limit).c_str());
	}
	else if (strcmp(argv[1], "metal-target-lookup-tag32-filter-persistent-bench") == 0)
	{
		const char* target_count_s = NULL;
		const char* query_count_s = NULL;
		const char* hits_s = NULL;
		...
		if (ReadOption(argc, argv, "--hits", &hits_s))
		{
			...
			hits = query_count / 64U;
		printf("%s\n", RCKMetalTargetLookupTag32FilterPersistentBenchJson(target_count, query_count, hits, min_ms, threadgroup_limit).c_str());
	}
	else if (strcmp(argv[1], "metal-target-lookup-tag16-filter-persistent-bench") == 0)
	{
		const char* target_count_s = NULL;
		const char* query_count_s = NULL;
		const char* hits_s = NULL;
		...
		if (ReadOption(argc, argv, "--hits", &hits_s))
		{
			...
			hits = query_count / 64U;
		printf("%s\n", RCKMetalTargetLookupTag16FilterPersistentBenchJson(target_count, query_count, hits, min_ms, threadgroup_limit).c_str());
	}
	else if (strcmp(argv[1], "metal-target-lookup-tag16-hash-filter-persistent-bench") == 0)
	{
		const char* target_count_s = NULL;
		const char* query_count_s = NULL;
		const char* hits_s = NULL;
		...
		if (ReadOption(argc, argv, "--hits", &hits_s))
		{
			...
			hits = query_count / 64U;
		printf("%s\n", RCKMetalTargetLookupTag16HashFilterPersistentBenchJson(target_count, query_count, hits, min_ms, threadgroup_limit).c_str());
	}
	...
	else if (strcmp(argv[1], "target-lookup-filter-build-bench") == 0)
	{
		const char* target_count_s = NULL;
		const char* iter_s = NULL;
		unsigned int target_count = 0;
		...
		printf("%s\n", RCKTargetLookupFilterBuildBenchJson(target_count, iterations).c_str());
	}
	else if (strcmp(argv[1], "target-lookup-tag32-build-from-keys-bench") == 0)
	{
		const char* target_count_s = NULL;
	...
	else if (strcmp(argv[1], "target-lookup-tag32-parity-parallel-insert-bench") == 0)
	{
		const char* target_count_s = NULL;
		const char* injected_count_s = NULL;
		const char* iter_s = NULL;
		...
		printf("%s\n", RCKTargetLookupTag32ParityParallelInsertBenchJson(target_count, injected_count, iterations).c_str());
	}
	...
	else if (strcmp(argv[1], "metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-target-lookup-tag32-bench") == 0)
	{
		...
		unsigned int min_ms = 0;
		unsigned int threadgroup_limit = 0;
		unsigned int lookup_threadgroup_limit = 0;
		unsigned int target_count = 0;
		unsigned int hits = 0;
	...
	else if (strcmp(argv[1], "metal-jacobian-dynamic-dp-stream-xyzz-affine-scan-target-lookup-tag32-rounds-bench") == 0)
	{
		...
		const char* lookup_repeat_s = NULL;
		const char* lookup_tg_s = NULL;
		const char* lookup_filter_bits_s = NULL;
		const char* lookup_filter_mode_s = NULL;
		const char* lookup_repeat_mode_s = NULL;
		const char* lookup_query_mode_s = "repeat";
		const char* walk_round_mode_s = "independent";
		const char* rounds_s = NULL;
		unsigned int iterations = 1024;
		unsigned int steps = 256;
		unsigned int jumps = 16;
		...
		unsigned int hits = 0;
		unsigned int lookup_repeat = 1;
		unsigned int lookup_filter_bits = 16;
		bool lookup_filter_mix = false;
		bool lookup_repeat_dedup = false;
		bool lookup_filter_bloom64 = false;
		...
		if (ReadOption(argc, argv, "--lookup-repeat-mode", &lookup_repeat_mode_s))
		{
			if (strcmp(lookup_repeat_mode_s, "full") == 0 || strcmp(lookup_repeat_mode_s, "repeat") == 0)
				lookup_repeat_dedup = false;
			else if (strcmp(lookup_repeat_mode_s, "dedup") == 0 || strcmp(lookup_repeat_mode_s, "dedup-repeat") == 0 || strcmp(lookup_repeat_mode_s, "dedup_repeat") == 0)
				lookup_repeat_dedup = true;
			else
			{
				PrintUsage();
				DeInitEc();
		...
		if (ReadOption(argc, argv, "--lookup-filter-bits", &lookup_filter_bits_s) &&
			(!ParseU32(lookup_filter_bits_s, &lookup_filter_bits) || (lookup_filter_bits != 16 && lookup_filter_bits != 32)))
		{
			PrintUsage();
			DeInitEc();
			return 1;
		}
		if (ReadOption(argc, argv, "--lookup-filter-mode", &lookup_filter_mode_s))
		{
			if (strcmp(lookup_filter_mode_s, "tag16") == 0)
			{
				lookup_filter_bits = 16;
				lookup_filter_mix = false;
				lookup_filter_bloom64 = false;
			}
			else if (strcmp(lookup_filter_mode_s, "tag16-mix") == 0)
			{
				lookup_filter_bits = 16;
				lookup_filter_mix = true;
				lookup_filter_bloom64 = false;
			}
			else if (strcmp(lookup_filter_mode_s, "tag32") == 0)
			{
				lookup_filter_bits = 32;
				lookup_filter_mix = false;
				lookup_filter_bloom64 = false;
			}
			else if (strcmp(lookup_filter_mode_s, "bloom64") == 0)
			{
				lookup_filter_bits = 16;
				lookup_filter_mix = false;
				lookup_filter_bloom64 = true;
			}
			else
			{
				PrintUsage();
		...
		ReadOption(argc, argv, "--jump-schedule", &jump_schedule_s);
		ReadOption(argc, argv, "--walk-round-mode", &walk_round_mode_s);
		printf("%s\n", RCKMetalJacobianDynamicDpStreamXyzzAffineScanTargetLookupTag32RoundsBenchJson(iterations, steps, jumps, rounds, target_count, hits, lookup_repeat, threadgroup_limit, dp_bits, jump_schedule_s, lookup_threadgroup_limit, lookup_filter_bits, lookup_filter_mix, lookup_repeat_dedup, lookup_query_mode_s, lookup_filter_bloom64, walk_round_mode_s).c_str());
	}
