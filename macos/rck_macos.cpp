...
static void PrintUsage()
{
	...
	printf("  rck_macos metal-target-lookup-tag32-packed-bench --target-count N --query-count N [--hits N] [--min-ms N] [--tg-limit N]\n");
...
int main(int argc, char* argv[])
{
	...
	else if (strcmp(argv[1], "metal-target-lookup-tag32-packed-bench") == 0)
	{
		const char* target_count_s = NULL;
		const char* query_count_s = NULL;
		const char* hits_s = NULL;
		...
		if (ReadOption(argc, argv, "--hits", &hits_s))
		{
			...
			hits = query_count / 64U;
		printf("%s\n", RCKMetalTargetLookupTag32PackedBenchJson(target_count, query_count, hits, min_ms, threadgroup_limit).c_str());
	}
