...
Fixed-round `distinct-misses` also reports `distinct_miss_source_seconds` and includes it in `setup_inclusive_seconds` and `setup_inclusive_wall_seconds`. This is the host time used to build compact deterministic miss sources and verify that they do not resolve as targets before the GPU hash-filter pass, so setup-inclusive metrics are the honest score for these probes.
