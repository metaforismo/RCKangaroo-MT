...
For massive target files, consider preparing uncompressed public keys with `python3 macos/prepare_targets.py stripped.txt -o targets.uncompressed.txt --uncompressed`. The preparer streams input and output, promotes the output atomically after validation, and avoids duplicate-tracking memory when `--keep-duplicates` is used for an already deduplicated list. The runtime parser already accepts both compressed and uncompressed secp256k1 public keys. Uncompressed files are larger, but they avoid recovering `y` with a square root for every compressed key during startup; on the MacBook Air M3 loader probe, the same 1,048,576 target fixture mapped with identical checksums at about `2.47M` targets/sec uncompressed versus about `198k` targets/sec compressed.
