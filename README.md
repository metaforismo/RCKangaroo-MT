...
Fixed-round `distinct-misses` also reports `distinct_miss_source_seconds` and includes it in `setup_inclusive_seconds` and `setup_inclusive_wall_seconds`. This is the host time used to build compact deterministic miss sources, verify that they do not resolve as targets, and produce the physical query hashes for the GPU hash-filter pass, so setup-inclusive metrics are the honest score for these probes. When a fused tag16 filter exists, the miss-source builder uses it as a host prefilter before exact `x256 + y_parity` checks; Bloom64 and filterless paths keep the original exact-check behavior. The default resolver trusts those already-validated compact miss sources and exact-checks only real DP-prefix positives; set `RCK_STRICT_DISTINCT_MISS_RESOLVE=1` to force the older full exact audit for every filter-positive compact miss while comparing checksums or debugging source-generation changes.
