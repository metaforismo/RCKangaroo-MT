...
- Metal smoke plus secp256k1 field-add, field-sub, field-double, field-neg, field-mul, and field-square microkernel checks for Apple Silicon runtime verification.
...
| macOS Metal | Early arithmetic backend | Builds and runs Metal smoke plus `field_add_mod_p`, `field_sub_mod_p`, `field_double_mod_p`, `field_neg_mod_p`, `field_mul_mod_p`, and `field_square_mod_p` microkernel checks when a Metal device is visible. |
...
- Added macOS target preparation, CPU oracle, CPU field arithmetic, Metal smoke, and Metal field-add/field-sub/field-double/field-neg/field-mul/field-square tools.
