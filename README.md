...
When `-start` is non-zero, target mapping uses chunked batch inversion on the host instead of one field inversion per target. This speeds the real startup phase reported as `Successfully mapped N targets against the Base Point` without changing the kangaroo walk, target identity, source-line reporting, or result verification. Target records keep the full affine point in 64 bytes; source lines are reconstructed from `index + 1` or a constant header base when possible, and fall back to an explicit `u32` side array only for non-dense files with comments or blank lines in the middle. On a 25,005,000-key stripped/header-only file, that avoids about 100,020,000 bytes of per-target source-line storage while preserving exact result line reporting. On macOS, `./macos/rck_macos target-set-load-bench --target-count 1048576 --start 2` isolates this parser plus start-offset mapping path; it is not a kernel GKeys/s benchmark.

For massive target files, consider preparing uncompressed public keys with `python3 macos/prepare_targets.py stripped.txt -o targets.uncompressed.txt --uncompressed`. The preparer streams input and output, promotes the output atomically after validation, and avoids duplicate-tracking memory when `--keep-duplicates` is used for an already deduplicated list. The runtime parser already accepts both compressed and uncompressed secp256k1 public keys. Uncompressed files are larger, but they avoid recovering `y` with a square root for every compressed key during startup; on the current MacBook Air M3 loader probe, the same 1,048,576 target fixture mapped with identical checksums at about `3.75M` targets/sec uncompressed versus about `201k` targets/sec compressed.
...
Fixed-round `distinct-misses` also reports `distinct_miss_source_seconds` and includes it in `setup_inclusive_seconds` and `setup_inclusive_wall_seconds`. This is the host time used to prepare deterministic miss sources or, on the default generated path, only the real DP-prefix hashes for the GPU hash-filter pass, so setup-inclusive metrics are the honest score for these probes. The default Metal path sends only real DP-prefix hashes and generates deterministic suffix miss hashes in the GPU filter kernel (`query_input=hash64_prefix_gpu_miss`). The resolver then exact-checks real DP-prefix positives and the much smaller set of GPU filter-positive generated suffix misses; if a generated suffix unexpectedly resolves as an exact target, the code falls back to the raw physical hash-buffer retry-source path. `RCK_STRICT_DISTINCT_MISS_RESOLVE=1` still forces exact auditing of raw compact miss sources when debugging fallback/source-generation changes.
