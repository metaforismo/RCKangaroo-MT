...
The runner executes correctness checks before benchmarks. Experiments may set `sample_runs`; when present, the runner executes the benchmark target repeatedly and records the selected metric median plus `<metric>_min`, `<metric>_max`, `sample_metric`, `sample_metric_values`, `sample_spread_ratio`, and `runner_sample_count` in `autoresearch/benchmarks.jsonl`.
...
python3 autoresearch/runner.py --experiment metal_jacobian_dynamic_xyzz_steps2048_dp6_walk_gate --budget-sec 240 --paired-baseline-ref HEAD --confirm-runs 2
python3 autoresearch/runner.py --experiment metal_jacobian_dynamic_xyzz_steps2048_dp6_walk_gate_131k --budget-sec 480 --paired-baseline-ref HEAD
...
against the accepted `steps512/dp8/repeat2048` path. The promoted
`steps2048_dp6_setup` gate repeats the same first-principles cadence step:
...
`steps=1024, dp_bits=7, lookup_repeat=1024` repeat path with
...
`setup_inclusive_ops_per_sec`, because `target_filter_build_seconds=0` on the
...
The M3 base-count by-base reduction gate compares the same command after the
Metal base-count kernel groups work by base DP query. Each thread still probes
one logical repeat, but positives are reduced inside threadgroup memory before
one global add per block/base. It scores runtime `ops_per_sec` and preserves
the same `base_query_count_repeated` encoding, checksum, hit count, and
false-positive count.

The fixed-round 25M base-count repeat gate moves that output representation
into the stricter rounds benchmark. It reuses distinct deterministic walk
batches and one target table, still probes every logical repeat on Metal, and
validates the same exact CPU `x256 + y_parity` equality plus repeated checksum.

The fixed-round 25M batched-walk gate keeps those distinct deterministic round
starts, but concatenates them into one larger Metal walk dispatch before
splitting results back into per-round affine scans, target offsets, and exact
lookup validation.
On the local M3 Air it passed two paired confirmations against the accepted
base-count rounds path with the same `target_lookup_checksum`, `hit_count`,
`filter_false_positive_count=0`, and `dp_count`. The kept rows measured
`ops_per_sec=109395855.173225` versus paired baseline `78109003.815734`
(`paired_speedup=1.400554`) and `59926669.794161` versus `52718596.332662`
(`paired_speedup=1.136727`). Treat the larger win as thermally noisy until
another hardware track reproduces it.

The M3 tag16-mix base-count gate is diagnostic, not a default policy. It uses
`--lookup-filter-mode tag16-mix` to test the same-memory mixed 16-bit filter
tag in the integrated command, including fused target-filter setup and the
base-count positive path. It scores `setup_inclusive_ops_per_sec` so a lookup
stage improvement cannot hide target-filter setup cost. Local paired
confirmation discarded it against standard tag16 while preserving checksum,
hit count, and zero false positives, so keep it for collision-distribution
experiments only.
